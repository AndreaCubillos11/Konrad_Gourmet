...
exports.ConsultarProductos = async (req, res, next) => {
  try {
    ...
    const productos = await Producto.findAll({
      attributes: ["id_producto", "nombre", "id_unidad", "id_categoria", "id_marca"]
    });
...
exports.ConsultarProductosPorCategoria = async (req, res, next) => {
  try {
    const { creador_id } = req.query;
    const { id_categoria } = req.params; // categoría por parámetro en la ruta
    ...
    if (creador_id) {
      await Auditoria.create({
        accion_registrada: "CONSULTAR PRODUCTOS POR CATEGORIA",
        id_usuario: creador_id
      });
    }
    else {
      return res.status(401).json({ error: "Creador_id no valido" });
    }
